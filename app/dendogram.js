...
dendroGram = function module() {
			...
			if( !option.labelHide){
            	...
            	if(option.treeType == 'linear'){
                	textLabel = chart.append("g")
                    	...
                    	.attr("height", 15)
                    	.attr("transform", function(d) { return "translate(" + 255 + "," + d.x + ")" })
    ...
    function changed() {
        ...
        if(option.treeType == 'linear'){
            ...
            d3.transition().duration(750).each(function() {
                ...
                linkExtension.transition().attr("d", function(d) {  return stepLinear(d.target.x, d.target.y, d.target.x,  checked ? 250 : d.target.y); });
                link.transition().attr("d", function(d) {  return stepLinear( d.source.x, d.source.y, d.target.x, d.target.y); });
                ...
                if( option.noLinkExtend == true){
                   linkExtension.transition().attr("d", function(d) {  return stepLinear(d.target.x, d.target.y, d.target.x,  checked ? d.target.y : d.target.y); });
                    textNode.transition().attr("transform", function(d) { return "translate(" + (d.y + 5) + "," +( d.x + 3)+ ")" })
                    textLabel.transition().attr("transform", function(d) { return "translate(" + (d.y + 3) + "," + d.x + ")" })
                    barChart.transition().attr("transform", function(d) { return "translate(" + 250 + "," + d.x + ")" })
                }
            });
        ...
        if(option.treeType == 'radial'){
             console.log("change")
            d3.transition().duration(750).each(function() {
              linkExtension.transition().attr("d", function(d) { return stepRadial(d.target.x, checked ? d.target.radius : d.target.y, d.target.x, innerRadius); });
              link.transition().attr("d", function(d) { return stepRadial(d.source.x, checked ? d.source.radius : d.source.y, d.target.x, checked ? d.target.radius : d.target.y) });
            });
	...
	*/
    function drawSunburst(){
		...
		// Reset height and cluster size for adding pie chart
            ...
            d3.transition().duration(750).each(function() {
                linkExtension.transition().attr("d", function(d) {  return stepLinear(d.target.x, d.target.y, d.target.x, d.target.y); });
                link.transition().attr("d", function(d) {  return stepLinear( d.source.x, d.source.y, d.target.x, d.target.y); });
                node.transition().attr("transform", function(d) { return "translate(" + d.y  + "," + d.x + ")"; })
                textLabel.transition().attr("transform", function(d) { return "translate(" + 265 + "," +( d.x )+ ")" })
                textNode.transition().attr("transform", function(d) { return "translate(" + 267 + "," +( d.x )+ ")" 
				})
    ...
    function addHeatMap(data){
        ...
        var termLable = chart.append("g").selectAll(".termlableg")
            ...
            .enter()
                ...
                .attr("transform", function(d,i){return "rotate(-45,"+(i*(gridWidth)+gridWidth )+",100)"})
		...
		if( option.heatType == "circle" || option.heatType == "punchCard"){
			...
			var heatmap = chart.selectAll(".cellg")
                ...
                .attr("transform", function(d,i){return "translate("+((gridWidth - 25)+',' +(gridHeight)+")"); })
                .attr("r", function(d){
		...
		for(var i = 0; i < 10; i++ ){
			...
			var heatMapSlice = chart.append("g")
    				...
    				.data(pieMap (nodes.filter(function(d){
  					...
  					.enter().append("path")
    				.attr("fill", function(d) {  return colorScale( heatData[i].value/2); })
     				.style("opacity", "0.8")
    				.attr("d", heatMapArc)
					.style("opacity", "0.4");
	...
	*/
	function alignData(aligndata){
		...
		for(var i =1; i<seq.length; i++){
			...
			seqObj.push({"name":seqArr[0], "seq": sequence} );
	...
	*/
    function addBarChart(bardata){
        ...
        barScale.domain([0, d3.max(bardata, function(d,i){ return d.value;})])
